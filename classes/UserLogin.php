...
class UserLogin
{
    ...
    public function check_user_login() {
        ...
        $query = $this->db->query('SELECT * FROM tb_user WHERE user_email = ? LIMIT 1', array($email));
        if (!$query) {
            $this->logged_in = false;
        ...
        if ($this->phpass->CheckPassword($password, $fetch['user_senha'])) {
            ...
            if ($post) {
                ...
                $_SESSION['userdata']['user_senha'] = $password;
                $_SESSION['userdata']['Session'] = $session_id;
                $_SESSION['couple_nome'] = null;
                $_SESSION['nome_noivos'] = null;
                $_SESSION['produtos'] = array();  
                $query = $this->db->query('UPDATE tb_user SET `Session` = ?, Last_Activity = CURRENT_TIMESTAMP WHERE User_Id = ?', array($session_id, $user_id));
            }
            $this->logged_in = true;
